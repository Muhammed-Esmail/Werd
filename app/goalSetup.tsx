	...
	const renderOption = (item: any, currentSelected: number, setter: (id: number) => void) => {
		return (
			<Pressable 
				...
				className={`flex-row items-center justify-between p-4 mb-3 rounded-2xl border-2 ${
					currentSelected === item.id ? "border-primaryGold bg-[#1A1A1A]" : "border-gray-800 bg-transparent"
				}`}
			>
				<View>
					<Text className={`${currentSelected === item.id ? "text-successGreen font-bold" : "text-gray-400"}`}>
						{item.text}
					</Text>
					<Text className={`${currentSelected === item.id ? "text-successGreen" : "text-gray-300"}`}>
						{item.description}
					</Text>
				</View>
				<View className={`h-5 w-5 rounded-full border-2 items-center justify-center ${
					currentSelected === item.id ? 'border-primaryGold' : 'border-gray-600'
				}`}>
					{currentSelected === item.id && <View className="h-2.5 w-2.5 rounded-full bg-primaryGold" />}
				</View>
			</Pressable>
		);
	}
return (
		...
		<SafeAreaView className="flex-1 bg-matteBlack">
            ...
            >
				...
				<RadioButton 
                    text='Custom' 
                    description='Choose a custom period' 
                    selected={selectedGoal === 4} 
                    onSelected={() => {
                        setPrevGoal(selectedGoal); 
                        setShow(true);
                    }} 
                />		
				...
				{selectedPartition === PartitionType.PAGE && (
